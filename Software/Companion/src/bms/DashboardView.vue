...
const activeFaults = computed(() => props.status ? setBits(props.status.bms_active_errors, bmsFaultNames) : [])
const hvFaults = computed(() => props.status ? setBits(props.status.hv_active_errors, hvReasonNames) : [])
...
<template>
  <main class="dashboard">
    <section class="dashboard-top">
      ...
      <section class="panel faults"><div class="panel-heading"><div><h2>Errors, HV reasons, and warnings</h2></div></div><div><h3>Active BMS errors</h3><p>{{ activeFaults.join(', ') || 'None reported' }}</p><h3>Latched BMS errors</h3><p>{{ status ? setBits(status.bms_latched_errors, bmsFaultNames).join(', ') || 'None reported' : 'Waiting for status' }}</p><h3>Active HV errors</h3><p>{{ hvFaults.join(', ') || 'None reported' }}</p><h3>Latched HV errors</h3><p>{{ status ? setBits(status.hv_latched_errors, hvReasonNames).join(', ') || 'None reported' : 'Waiting for status' }}</p><h3>Warnings</h3><p>{{ status ? setBits(status.warnings, warningNames).join(', ') || 'None reported' : 'Waiting for status' }}</p></div></section>
    </section>
    ...
    <ServiceView :transport="transport" :capabilities="capabilities" :connected="connected" :run-requested="status?.run_request ?? false" :balancing-requested="status?.balancing_request ?? false" :gateway="gateway" />
  </main>
</template>
