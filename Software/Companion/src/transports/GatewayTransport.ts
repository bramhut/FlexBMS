...
export class GatewayTransport implements BmsTransport {
  ...
  private handleMessage(message: MessageEvent): void {
    ...
    if (parsed.type === 'snapshot') { const snapshot = { status: this.normaliseBmsStatus(parsed.status as Status), pack: parsed.pack, ...(parsed.energy ? { energy: parsed.energy } : {}), ...(parsed.hv_voltages ? { hv_voltages: parsed.hv_voltages } : {}), ...(parsed.goodwe_can ? { goodwe_can: parsed.goodwe_can } : {}), cells: parsed.cells, temperatures: parsed.temperatures } as Snapshot; this.lastBmsStatus = snapshot.status; this.statusListeners.forEach(listener => listener(snapshot.status)); this.snapshotListeners.forEach(listener => listener(snapshot)) }
