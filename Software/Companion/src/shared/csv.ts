...
export function csvHeader(snapshot: Snapshot): string[] {
  const fields = ['timestamp_ms', 'pack_voltage_v', 'pack_current_a', 'soc_valid', 'soc_percent']
...
export function csvRow(snapshot: Snapshot): string[] { return [String(Date.now()), cellVoltageV(snapshot.pack.pack_voltage_uV).toFixed(6), currentA(snapshot.pack.pack_current_raw).toFixed(4), snapshot.status.soc_valid ? 'true' : 'false', snapshot.status.soc_valid ? socPercent(snapshot.pack.soc_raw).toFixed(3) : '', ...snapshot.cells.flatMap(cell => cell.cell_voltage_uV.map(value => cellVoltageV(value).toFixed(6))), ...snapshot.temperatures.flatMap(temp => [...temp.ntc_raw.map(value => ntcCelsius(value).toFixed(3)), icCelsius(temp.ic_temp_raw).toFixed(3)])] }
